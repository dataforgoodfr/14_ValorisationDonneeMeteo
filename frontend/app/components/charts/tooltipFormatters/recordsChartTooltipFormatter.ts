...
function formatBarTooltip(param: DefaultLabelFormatterCallbackParams): string {
    ...
    const hotMarker = `<span style="display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:${TEMPERATURE_COLORS.hot};"></span>`;
    const coldMarker = `<span style="display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:${TEMPERATURE_COLORS.cold};"></span>`;
    return [
        `<b>${data.period}</b>`,
