...
function formatBarTooltip(param: DefaultLabelFormatterCallbackParams): string {
    ...
    const hotMarker = `<span style="display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:${COLORS.value.cold};"></span>`;
    const coldMarker = `<span style="display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:${COLORS.value.hot};"></span>`;
    return [
        `<b>${data.period}</b>`,
