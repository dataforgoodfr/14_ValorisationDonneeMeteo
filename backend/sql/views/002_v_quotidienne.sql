...
WITH horaire_from_infrahoraire AS (SELECT geo_id_insee                                                                AS station_id,
     ...
     horaire_temps_reel AS (SELECT geo_id_insee  AS station_id,
                                   validity_time AS timestamp,
                                   t             AS t,
                                   tn            AS tn,
                                   tx            AS tx
                            FROM "HoraireTempsReel"
                            WHERE date_trunc('hour', now()) - interval '36 hours' <= validity_time
                              AND validity_time <= date_trunc('hour', now()) - interval '3 hours'),

     horaire AS (SELECT "NUM_POSTE"  AS station_id,
                        "AAAAMMJJHH" AS timestamp,
                        "T"          AS t,
                        "TN"         AS tn,
                        "TX"         AS tx
                 FROM "Horaire"
                 WHERE date_trunc('day', now()) - interval '4 days' <= "AAAAMMJJHH"
                   AND "AAAAMMJJHH" < date_trunc('hour', now()) - interval '36 hours'),

     combined_horaire AS (SELECT station_id, timestamp, t, tn, tx
                          FROM horaire_from_infrahoraire
                          UNION ALL
                          SELECT station_id, timestamp, t, tn, tx
                          FROM horaire_temps_reel
                          UNION ALL
                          SELECT station_id, timestamp, t, tn, tx
                          FROM horaire),

     tn_quotidienne_temps_reel AS (SELECT station_id                                                                                  AS station_id,
     ...
     tx_quotidienne_temps_reel AS (SELECT station_id                                                              AS station_id,
     ...
     tn_tx_quotidienne_temps_reel AS (SELECT station_id, timestamp, tn, NULL::numeric AS tx
                                      FROM tn_quotidienne_temps_reel
                                      UNION ALL
                                      SELECT station_id, timestamp, NULL::numeric AS tn, tx
                                      FROM tx_quotidienne_temps_reel),

     quotidienne_temps_reel AS (SELECT station_id,
                                       timestamp,
     ...
     quotidienne AS (SELECT "NUM_POSTE" AS station_id,
     ...
     combined_quotidienne AS (SELECT station_id, timestamp, tntxm, tn, tx
                              FROM quotidienne_temps_reel
                              UNION ALL
                              SELECT station_id, timestamp, tntxm, tn, tx
                              FROM quotidienne)
