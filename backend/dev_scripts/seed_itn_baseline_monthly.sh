...
CSV_PATH="${1:-${ROOT_DIR}/db_data/itn_baseline_monthly_9120.csv}"
TABLE_NAME="${TABLE_NAME:-v_itn_baseline_monthly_1991_2020}"
...
CREATE TABLE public.${TABLE_NAME} (
    ...
    CONSTRAINT pk_${TABLE_NAME} PRIMARY KEY (month)
);
SQL
...
psql -h "$DB_HOST" \
     ...
     -d "$DB_NAME" \
     -v ON_ERROR_STOP=1 <<SQL
\copy public.${TABLE_NAME} (month, sample_size, itn_mean, itn_stddev, itn_p20, itn_p80) FROM '${CSV_PATH}' WITH (FORMAT csv, HEADER true)
SQL
...
echo "Monthly baseline seed applied."
