...
#
#   --cutoff YYYY-MM-DD  Cutoff date à stocker dans mv_records_absolus_meta.
...
META_SQL="${ROOT_DIR}/sql/schemas/records/001_mv_records_absolus_meta.sql"

[[ -f "${META_SQL}" ]] || { echo "Missing file: ${META_SQL}" >&2; exit 1; }

echo "Creating mv_records_absolus_meta..."
psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" \
     -v ON_ERROR_STOP=1 \
     -f "${META_SQL}"
...
if [[ -n "${CUTOFF_ARG}" ]]; then
    ...
    psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" \
         -v ON_ERROR_STOP=1 \
         -c "TRUNCATE public.mv_records_absolus_meta; INSERT INTO public.mv_records_absolus_meta (cutoff_date) VALUES ('${CUTOFF_ARG}');"
else
    echo "Storing cutoff date (auto: MAX(AAAAMMJJ) from Quotidienne)..."
    psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" \
         -v ON_ERROR_STOP=1 <<'SQL'
TRUNCATE public.mv_records_absolus_meta;
...
psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" \
     -c "SELECT cutoff_date FROM public.mv_records_absolus_meta;"
