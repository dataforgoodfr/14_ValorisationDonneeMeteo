...
"${psql_base[@]}" -c 'SELECT COUNT(*) AS v_station_count FROM public.v_station_qualifiee_hexagone;'
"${psql_base[@]}" -c 'SELECT COUNT(*) AS v_quotidienne_itn_count FROM public.v_quotidienne;'
"${psql_base[@]}" -c 'SELECT MIN(date), MAX(date) FROM public.v_quotidienne;'
"${psql_base[@]}" -c 'SELECT COUNT(*) AS baseline_station_daily_mean_1991_2020_count FROM public.mv_baseline_station_daily_mean_1991_2020;'
"${psql_base[@]}" -c 'SELECT * FROM public.mv_baseline_station_daily_mean_1991_2020 ORDER BY station_code, month, day LIMIT 5;'
"${psql_base[@]}" -c 'SELECT COUNT(*) AS itn_baseline_count FROM public.v_itn_baseline_daily_1991_2020;'
"${psql_base[@]}" -c 'SELECT COUNT(*) AS itn_baseline_monthly_count FROM public.v_itn_baseline_monthly_1991_2020;'
"${psql_base[@]}" -c 'SELECT COUNT(*) AS itn_baseline_yearly_count FROM public.v_itn_baseline_yearly_1991_2020;'
"${psql_base[@]}" -c 'SELECT * FROM public.v_itn_baseline_monthly_1991_2020 ORDER BY month LIMIT 5;'
"${psql_base[@]}" -c 'SELECT * FROM public.v_itn_baseline_yearly_1991_2020;'
