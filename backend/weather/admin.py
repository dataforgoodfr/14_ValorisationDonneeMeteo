...
class StationAdmin(admin.ModelAdmin):
    list_display = ["id", "nom", "departement", "lat", "lon", "alt"]
...
class HoraireTempsReelAdmin(admin.ModelAdmin):
    list_display = ["geo_id_insee", "validity_time", "t", "u", "ff", "rr1"]
    list_filter = ["geo_id_insee"]
    ordering = ["-validity_time"]


@admin.register(Quotidienne)
class QuotidienneAdmin(admin.ModelAdmin):
    list_display = ["num_poste", "nom_usuel", "date", "tn", "tx", "rr"]
    list_filter = ["num_poste"]
    ordering = ["-date"]
